...
void solution()
{
    ...
    for (ll i = 0; i < n; i++)
    {
        ll x, y;
        cin >> x >> y;
        ll s = pow(x, 2) + pow(y, 2);
        double d = double(sqrt(s));
        ll org = ceil(d);
        if (org <= rs + rh)
        {
            val1++;
    ...
    /*
    this is a correspondent
    aryan is a big boy
    
    strong assssss sdfjlstjlrtljss
    sdroiroiios;sd;;dpojoptrhioslfkll*/

    sort(all(uwu));
    sort(all(kaka));

    ll si1 = uwu.size(), si2 = kaka.size();

    if (uwu[0] < kaka[0])
    {
        ll i = 0;
        while (uwu[i] < kaka[0] && i < si1)
        {
            i++;
            val1++;
        }
